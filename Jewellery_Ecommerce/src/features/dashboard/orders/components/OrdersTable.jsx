  ...
  return (
    <div className="overflow-x-auto">
      <table className="min-w-full border-collapse text-left text-sm text-zinc-800">
        ...
        <tbody>
          {orders.map((order) => (
            <tr key={order.id} className="border-b border-zinc-200/80 align-middle">
              <td className="px-6 py-5 text-base">{order.id}</td>
              <td className="px-6 py-5 text-base">{formatDate(order.placedAt)}</td>
              <td className="px-6 py-5">
                <div className="flex items-center gap-3">
                  <div className="flex h-12 w-12 items-center justify-center rounded-md border border-zinc-200 bg-white">
                    <svg className="h-6 w-6 text-amber-600" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                  ...
                  <div>
                    <p className="text-sm font-medium text-zinc-700">{order.items[0].name}</p>
                    <p className="text-xs text-zinc-500">{order.items.length} item</p>
                  </div>
                </div>
              </td>
              <td className="px-6 py-5">
