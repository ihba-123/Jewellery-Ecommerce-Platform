...
const OTPVerification = () => {
  ...
  const handleVerify = async (e) => {
    ...
    try {
      ...
      if (otpCode === "123456") {
        // Store user with their type/mode
        const userData = {
          ...signupData,
          type: userMode,
        ...
        const redirectPath = userMode === "kaligard"
          ? "/kaligard-dashboard/profile"
          : "/dashboard";
      ...
      <div className="w-full max-w-md rounded-3xl border border-white/12 bg-white/8 shadow-[0_20px_60px_rgba(0,0,0,0.28)] backdrop-blur-2xl p-5 sm:p-8 text-white">
        ...
        <div className="mb-8 text-center">
          ...
          <h1 className="mt-2 text-2xl sm:text-3xl font-bold text-white">
            Verify Your {userMode === "business" ? "Business" : userMode === "kaligard" ? "Kaligard" : ""} Account
          </h1>
          <p className="mt-3 text-sm text-white/70">
            We've sent a 6-digit verification code to your number. Enter it
        ...
        <form onSubmit={handleVerify} className="grid gap-6">
          <div className="grid gap-4">
            ...
            <div
              className="flex gap-2 sm:gap-3 justify-center sm:justify-between"
              onPaste={handlePaste}
            >
              {otp.map((digit, index) => (
                <input
            ...
            {error && (
              <span className="text-xs text-rose-400 text-center mt-2">
                {error}
              </span>
            )}
          </div>
