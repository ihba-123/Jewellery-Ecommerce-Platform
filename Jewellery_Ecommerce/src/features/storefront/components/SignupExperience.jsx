...
export default function SignupExperience({ mode = "customer" }) {
  ...
  const errors = useMemo(() => {
    ...
    if (formValues[emailKey] && !isValidEmail(formValues[emailKey]))
      e[emailKey] = "Invalid email";
      ...
      <div className="w-full max-w-3xl rounded-3xl border border-white/12 bg-white/8 shadow-[0_20px_60px_rgba(0,0,0,0.28)] backdrop-blur-2xl p-5 sm:p-8 text-white">
        ...
        <div className="mb-6 grid grid-cols-3 rounded-2xl bg-white/6 p-1 gap-1 border border-white/10">
          <Link
            ...
            className={`rounded-xl px-4 py-2.5 text-center text-sm font-semibold transition-all duration-200 ${activeMode === "customer" ? "bg-white/12 text-white border border-white/10" : "text-white/65 hover:text-white hover:bg-white/6"}`}
          >
            Customer
          </Link>
          <Link
